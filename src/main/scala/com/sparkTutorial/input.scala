...
object input {
  ...
  def createDataFrames(spark: SparkSession, jdbcReader: DataFrameReader): Unit = {
    ...
    queries.map({ case (view_name, sourceType, query) =>
      println("******************************************************************")
      println(s"---- Creating dataframe for $view_name from $sourceType using query/collection name provided----")
      ...
      println(s"---- Creating df for $view_name from $sourceType ----")
      df.createOrReplaceTempView(view_name)
      println(s"---- Created df for $view_name from $sourceType ----")
      (view_name, df) //this is how return works in scala
    })
  }

  def getData(spark: SparkSession): Unit = {
    println(s"---- Invoked getData for running transformations ----")
